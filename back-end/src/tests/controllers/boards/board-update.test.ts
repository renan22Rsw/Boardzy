...
describe("Board controller update action", () => {
  ...
  it("should update the board title and return a sucess message with status code of 200", async () => {
    (clerkClient.users.getUser as jest.Mock).mockResolvedValue(userMock);

    boardService.updateBoardTitle(boardMock.id, boardMock.title);
    ...
    await boardController.updateBoardTitle(
    ...
    expect(res.send).toHaveBeenCalledWith({
      message: `The title of your board ${boardMock.title} has been updated`,
    });
  });

  it("should not update the board title and return a error message with status code of 400", async () => {
    (clerkClient.users.getUser as jest.Mock).mockResolvedValue(userMock);

    boardService.updateBoardTitle.mockRejectedValue(
  ...
  it("should not update the board title and return a error message with status code of 500", async () => {
    boardService.updateBoardTitle.mockRejectedValue("Unexpected error");
